...
alias gopher {
  ; the command line pary of the gopher client, it can open the browser
  ...
  if ($dialog(gopher) == $null) {
    ...
    .write -l $+ %gopher_client.history.current $mircdir $+ script\gopher\history %gopher_client.current
  ...
  if (. isin %gopher_client.file_to_get  ) {
    if  ($file_type($gettok(%gopher_client.file_to_get,$numtok(%gopher_client.file_to_get,46),46)) != txt) {
      if ($exists($getdir $+ $gettok(%gopher_client.file_to_get,$numtok(%gopher_client.file_to_get,47),47)) == $true) {
        ...
        .splay -w $mircdir $+ script\gopher\chord.wav
        if ( $dialog(gopher_overwrite,gopher_overwrite,-4) == $null) {
          .write $mircdir $+ script\gopher\cache\ $+ %gopher_client.history.current 
          .write $mircdir $+ script\gopher\gopher.tmp
          .did -r gopher 12
...
on *:DIALOG:gopher:init:0:{
  ...
  while ( %gopher_client.init.count <= $lines($mircdir $+ script\gopher\history) ) {
    .remove $mircdir $+ script\gopher\cache\ $+ %gopher_client.init.count
    inc %gopher_client.init.count
  }

  ; make sure the cache is clear

  .remove $mircdir $+ script\gopher\history
  .remove $mircdir $+ script\gopher\gopher.tmp
...
on *:dialog:gopher:sclick:1:{
  ...
  while ( %gopher_client.close.count <= $lines($mircdir $+ script\gopher\history) ) {
    .remove $mircdir $+ script\gopher\cache\ $+ %gopher_client.close.count
...
on *:dialog:gopher:sclick:2:{
  ...
  %gopher_client.current = $read( $mircdir $+ script\gopher\history,%gopher_client.history.current )

  ; update current

  did -r gopher 7
  did -a gopher 7 gopher:// $+ $read( $mircdir $+ script\gopher\history,%gopher_client.history.current)
  ...
  ;launch.gopher $read( $mircdir $+ script\gopher\history,%gopher_client.history.current )
  ...
  .copy $mircdir $+ script\gopher\cache\ $+ %gopher_client.history.current $mircdir $+ script\gopher\gopher.tmp

  var %gopher_client.back.count = 1

  while (%gopher_client.back.count <= $lines($mircdir $+ script\gopher\gopher.tmp)) {
    did -a gopher 9 $format_output($read($mircdir $+ script\gopher\cache\ $+ %gopher_client.history.current,nt, %gopher_client.back.count))
...
on *:dialog:gopher:sclick:3:{
  ...
  while (%gopher_client.forawrd.count <= $lines($mircdir $+ script\gopher\gopher.tmp)) {
    did -a gopher 9 $format_output($read($mircdir $+ script\gopher\cache\ $+ %gopher_client.history.current,nt, %gopher_client.forawrd.count))
...
on *:dialog:gopher:sclick:8:{
  ...
  if ($remove($did(gopher,7),gopher://) != %gopher_client.current ) {
    ...
    inc %gopher_client.history.current
    .write -l $+ %gopher_client.history.current $mircdir $+ script\gopher\history $remove($did(gopher,7),gopher://)
    %gopher_client.history.max = %gopher_client.history.current
...
on *:DIALOG:gopher:dclick:9:{
  ...
  var %gopher_client.seltext = $remtok($read($mircdir $+ script\gopher\gopher.tmp,$did(gopher,9,1).sel),+,9) 
...
on *:DIALOG:gopher:menu:15:{
  ...
  if (%gopher_client.save.file != $null ) .copy $mircdir $+ script\gopher\gopher.tmp %gopher_client.save.file
}
...
dialog about_gopher {
  ...
  icon 10, 80 10 32 32, $mircdir $+ script\gopher\gopher.ico
  text "Written by ValkyGrrl",18, 5 50 50 8
  link "<valkygrrl@yahoo.com>",19, 56 50 60 8
  box "",20, 1 -1 119 74
...
on *:DIALOG:about_gopher:sclick:19: {
  ...
  .run mailto:valkygrrl@yahoo.com
  return
...
on *:dialog:favorites:init:0:{
  ...
  var %favorite.init.element $read -l $+ %favorite.init.counter $mircdir $+ script\gopher\favorites

  ; fill the listbox with the elements from the favorites file

  while ( $null != %favorite.init.element ) {
    did -a favorites 5 gopher:// $+ $read -l $+ %favorite.init.counter $mircdir $+ script\gopher\favorites
...
on *:dialog:favorites:sclick:2:{
  ...
  if ( %favorite.add.element_to_add != $null ) {
    ...
    .write $mircdir $+ script\gopher\favorites %favorite.add.element_to_add
    did -r favorites 1
    ...
    var %favorite.element $read -l $+ %favorite.counter $mircdir $+ script\gopher\favorites

    while ( $null != %favorite.element ) {
      did -a favorites 5 gopher:// $+ $read -l $+ %favorite.counter $mircdir $+ script\gopher\favorites
...
on *:dialog:favorites:sclick:3:{
  ...
  .write -ds $+ " $+ $remove($did(favorites,5).seltext,gopher://) $+ " $mircdir $+ script\gopher\favorites 
  ...
  var %favorite.delete.element $read -l $+ %favorite.delete.counter $mircdir $+ script\gopher\favorites

  while ( $null != %favorite.delete.element ) {
    did -a favorites 5 gopher:// $+ $read -l $+ %favorite.delete.counter $mircdir $+ script\gopher\favorites
...
on *:dialog:favorites:dclick:5:{
  ...
  inc %gopher_client.history.current
  .write -l $+ %gopher_client.history.current $mircdir $+ script\gopher\history $remove($did(favorites,5).seltext,gopher://)
  %gopher_client.history.max = %gopher_client.history.current
...
on *:SOCKOPEN:gopher_client:{
  ...
  .remove $mircdir $+ script\gopher\gopher.tmp
  .remove $mircdir $+ script\gopher\cache\ $+ %gopher_client.history.current
  ...
  if ($sockerr > 0) {
    ...
    sockclose gopher 
    .splay -w $mircdir $+ script\gopher\click.wav
    did -a gopher 9 Cannot connect to server
    did -r gopher 12
    did -a gopher 12 Error: Server not found
    .write $mircdir $+ script\gopher\gopher.tmp Cannot connect to server
    .write $mircdir $+ script/gopher/cache/ $+ %gopher_client.history.current Cannot connect to server
    return

    ; display an error message, update the cache and return without doign more
  }
  else {
...
on *:SOCKREAD:gopher_client:{
  ...
  if (%gopher_client.filename == $null) {
    ...
    bwrite $mircdir $+ script\gopher\gopher.tmp -1 -1 &gopher_client.buffer

    ; store the data the isn't displayed in the listbox

    bwrite $mircdir $+ script\gopher\cache\ $+ %gopher_client.history.current -1 -1 &gopher_client.buffer
...
on *:sockclose:gopher_client:{
  ...
  if (%gopher_client.filename == $null) {
    var %temp = 1
    while ( %temp <= $lines($mircdir $+ script\gopher\gopher.tmp) ) {
      .did -a gopher 9 $format_output($read($mircdir $+ script\gopher\gopher.tmp,nt, %temp))
      inc %temp
    }
    ; diaplay the text relivent to the user in the listbox
  }
  else .splay -w $mircdir $+ script\gopher\chord.wav
